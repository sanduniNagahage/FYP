...
def predict_masking(masking_sentences):

    # dataframe = pd.read_csv(prep_csv)
    response = []
    for current in range(len(masking_sentences)):
        print(current)
        # if current < start:
        #     print('skip')
        #     current = current + 1
        ...
        index, sentence, prepositions = masking_sentences[current]
        # print()
        ...
        for prep in rearranged_prepositions:
            ...
            # print("Predicted sentence :", predicted_sentence)
            predicted_sentecnes.append(predicted_sentence)
        ...
        response.append([index, sentence, predicted_sentecnes,prediction_accuracy])
